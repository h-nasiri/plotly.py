...
class ValidateResponseTest(TestCase):
    ...
    def test_validate_ok(self):
        try:
            utils.validate_response(self.get_response(content='{}'))
    ...
    def test_validate_no_content(self):
        ...
        response = self.get_response(content='', status_code=200)
        try:
        ...
        response = self.get_response(content=None, status_code=200)
        try:
            utils.validate_response(response)
        except PlotlyRequestError as e:
            self.assertEqual(e.message, 'No Content')
    ...
    def test_validate_non_json_content(self):
        response = self.get_response(content='foobar', status_code=200)
        try:
            utils.validate_response(response)
        except PlotlyRequestError as e:
